...
describe("database schema and seed", () => {
  ...
  it("persists and edits one validated league configuration per user", async () => {
    ...
    await db.query(
      ...
      ) values ($1, 'Original League', 12, 'snake', 3, 'ppr', 1, 2, 2, 1, 1, 0, 1, 1, 6)
    ...
    await db.query(
      `insert into league_configurations (
        ...
        qb_slots, rb_slots, wr_slots, te_slots, flex_slots, superflex_slots,
        k_slots, dst_slots, bench_slots
      ) values ($1, 'Edited League', 12, 'snake', 3, 'half_ppr', 1, 2, 2, 1, 1, 0, 1, 1, 6)
      on conflict (user_id) do update set
        name = excluded.name, scoring_preset = excluded.scoring_preset`,
      [userId],
    );

    const result = await db.query<{ name: string; scoring_preset: string }>(
      "select name, scoring_preset from league_configurations where user_id = $1",
      [userId],
    );
    expect(result.rows).toEqual([
      { name: "Edited League", scoring_preset: "half_ppr" },
    ]);
  });
  ...
  it("rejects invalid league configurations at the database boundary", async () => {
    await expect(
      db.exec(`insert into league_configurations (
        ...
        'snake', 11, 'ppr', 1, 2, 2, 1, 1, 0, 1, 1, 6
      )`),
    ).rejects.toThrow();
    ...
    await expect(
      db.exec(`insert into league_configurations (
        ...
        'linear', 1, 'standard', 0, 0, 0, 0, 0, 0, 0, 0, 6
      )`),
    ).rejects.toThrow();
  });
