...
4. Valid records are persisted with their raw source values; invalid records
   are quarantined with validation errors.
...
valid snapshot remains available. If only some rows fail, valid rows are stored
...
The fixture adapter under `providers/fixture/` is the executable example. Real
FantasyPros, FantasyNerds, nflverse, and future adapters should not require
